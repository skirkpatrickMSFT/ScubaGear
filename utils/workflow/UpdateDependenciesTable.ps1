...
function Update-DependenciesTable {
    ...
    Write-Host "Updating dependencies table from $RequiredVersionsPath to $DependenciesPath"
    ...
    Write-Host "Found $($ModuleList.Count) modules to update"
    ...
    foreach ($module in $ModuleList) {
        ...
        if ($DependenciesContent -match $ExistingRowPattern) {
            ...
            Write-Host "✅ Updated row for module: $($module.ModuleName)"
            $updatedCount++
        } else {
    ...
    Write-Host "✅ Successfully updated $updatedCount out of $($ModuleList.Count) modules in dependencies.md"
...
# Main execution
try {
    $result = Update-DependenciesTable -RequiredVersionsPath $RequiredVersionsPath -DependenciesPath $DependenciesPath
    Write-Host "Dependencies table update completed successfully"
    exit 0
}
catch {
